...
def dashboard_page(
    *,
    ...
    template_options = "".join(f'<option value="{html.escape(str(item["id"]), quote=True)}">{html.escape(str(item["name"]))} · {html.escape(str(item["group_name"]))}</option>' for item in policy_templates)
    template_cards = "".join(f'<article class="policy-template"><header><div><strong>{html.escape(str(item["name"]))}</strong><small>{html.escape(str(item["group_name"]))}</small></div><span class="{'posture-badge' if item.get("protected") else 'template-custom'}">{'Built in' if item.get("protected") else 'Custom'}</span></header><p>{html.escape(str(item["description"]))}</p><button type="button" class="quiet" data-select-template="{html.escape(str(item["id"]), quote=True)}">Use this template</button></article>' for item in policy_templates)
    rows: list[str] = []
    for user in users:
    ...
    for device in devices:
        ...
        device_rows.append(
            f"""<tr>
              ...
              <td><button type="button" class="table-action" data-create-device data-user-id="{html.escape(user_ids.get(owner, ''), quote=True)}" data-user-name="{html.escape(owner, quote=True)}">{_icon('plus')}<span>Add another device</span></button></td>
            </tr>"""
        )
    ...
    health_rows = "".join(f'''<article class="service-health-check {html.escape(str(item.get("status", "unavailable")), quote=True)}"><span class="health-check-led" aria-hidden="true"></span><div><strong>{html.escape(str(item.get("name", "Health check")))}</strong><small>{html.escape(str(item.get("impact", "")))}</small><p><b>Safe next step:</b> {html.escape(str(item.get("remediation", "No action needed.")))}</p></div></article>''' for item in health_checks) or '<div class="empty"><strong>No health data is available yet.</strong><span>Refresh this view to check the current router state.</span></div>'
  ...
  <section class="winbox-main">
    <main class="workspace-content">
      ...
      <section class="app-view" id="service-health" data-view="service-health" hidden>
        ...
        <section class="metric-grid service-health-summary" aria-label="Service health summary"><article class="metric"><div class="metric-icon">{_icon('system')}</div><div><small>Overall state</small><strong data-service-health-overall>{health_overall_label}</strong><span data-service-health-checked>{len(health_checks)} checks completed</span></div></article><article class="metric"><div class="metric-icon">{_icon('enable')}</div><div><small>Healthy</small><strong data-service-health-count="healthy">{health_counts['healthy']}</strong><span>Operating as expected</span></div></article><article class="metric"><div class="metric-icon">{_icon('shield')}</div><div><small>Needs review</small><strong data-service-health-count="warning">{health_counts['warning']}</strong><span>Review before changing RouterOS</span></div></article><article class="metric"><div class="metric-icon">{_icon('disable')}</div><div><small>Unavailable</small><strong data-service-health-count="unavailable">{health_counts['unavailable']}</strong><span>Could not be verified</span></div></article></section>
      ...
      <section class="app-view" id="policy-templates" data-view="policy-templates" hidden>
        <header class="view-heading"><div><p class="eyebrow">ACCESS GOVERNANCE</p><h1>Policy Templates</h1><p>Preview exact differences before applying selected policy to selected VPN users.</p></div><button type="button" class="primary" data-open-template-create>{_icon('plus')}<span>New custom template</span></button></header>
        <section class="policy-intro"><span>{_icon('shield')}</span><div><strong>Nothing changes until you apply it.</strong><small>Template application creates a RouterOS checkpoint, modifies selected users only, and records the result in Change History.</small></div></section>
        <section class="policy-template-grid">{template_cards}</section>
        <section class="panel template-apply-panel"><div class="panel-heading"><div>{_icon('users')}<span><strong>Preview and apply</strong><small>Direct user edits remain intentional overrides.</small></span></div></div><form data-template-apply><label><span>Template</span><select name="template_id" required><option value="">Select a template</option>{template_options}</select></label><fieldset><legend>Selected VPN users</legend><div class="template-user-list">{''.join(f'<label><input type="checkbox" name="user_ids" value="{html.escape(str(user.get("id", "")), quote=True)}"><span><strong>{html.escape(str(user.get("name", "")))}</strong></span></label>' for user in users)}</div></fieldset><p class="form-status" role="status"></p><div class="template-preview" hidden data-template-preview></div><footer><button type="button" class="quiet" data-template-preview-button>Preview changes</button><button type="submit" class="primary" disabled data-template-apply-button>Apply to selected users</button></footer></form></section>
      </section>
...
<dialog id="template-dialog"><form id="template-form" method="dialog" class="dialog-card"><header><div><p class="eyebrow">ACCESS GOVERNANCE</p><h2>Create custom policy template</h2><p>Save a reusable set of VPN controls. It does not alter users yet.</p></div><button type="button" class="icon" data-close aria-label="Close">×</button></header><div class="dialog-fields"><label><span>Template name</span><input name="name" required minlength="2" maxlength="48"></label><label><span>Group name</span><input name="group_name" required minlength="2" maxlength="48"></label><label><span>Description</span><input name="description" required minlength="1" maxlength="180"></label><label><span>Traffic policy</span><select name="policy"><option value="full-tunnel">Full tunnel</option><option value="lan-only">LAN only</option><option value="internet-only">Internet only</option></select></label><label><span>Concurrent devices</span><select name="max_sessions"><option value="1">1 device</option><option value="2">2 devices</option><option value="3">3 devices</option><option value="4">4 devices</option><option value="5" selected>5 devices</option></select></label><label><span>Speed limit</span><select name="rate_limit_kbps"><option value="0">Unlimited</option><option value="5120">5 Mbps</option><option value="10240">10 Mbps</option><option value="25600">25 Mbps</option><option value="51200">50 Mbps</option><option value="102400">100 Mbps</option></select></label><label><span>Data quota</span><select name="quota_mb"><option value="0">Unlimited</option><option value="1024">1 GB / month</option><option value="5120">5 GB / month</option><option value="10240">10 GB / month</option></select></label><label><span>Schedule</span><select name="schedule"><option value="always">Always allowed</option><option value="weekdays">Weekdays</option><option value="daytime">Every day · daytime</option></select></label><label><span>VPN DNS</span><select name="dns_mode"><option value="router">RouterOS DNS</option><option value="cloudflare">Cloudflare DNS</option></select></label><label class="toggle"><input type="checkbox" name="notifications" checked><span><strong>Show automated alerts</strong></span></label></div><p class="form-status" role="status"></p><footer><button type="button" class="quiet" data-close>Cancel</button><button type="submit" class="primary">Save template</button></footer></form></dialog>
"""
