...
def dashboard_page(
    *,
    ...
    crl_ready = bool(certificate_settings.get("crl_ready"))
    ...
    <main class="workspace-content">
      ...
      <section class="app-view" id="overview" data-view="overview">
        ...
        <section class="enterprise-grid" aria-label="Service health and security posture">
          <article class="panel operations-panel"><div class="panel-heading"><div>{_icon('health')}<span><strong>Service health</strong><small>Live MikroTik capacity and VPN availability</small></span></div><div class="panel-heading-actions"><span class="posture-badge">{health_overall_label}</span><button type="button" class="quiet" data-view-target="service-health">Open checks</button></div></div><div class="health-grid">
            <div class="health-item"><span>VPN service</span><strong class="health-state {'good' if server_enabled else 'bad'}"><i></i>{'Online' if server_enabled else 'Offline'}</strong><div class="vpn-protocol"><span>{html.escape(str(ovpn_server.get('protocol', '')).upper())}</span><code>{int(ovpn_server.get('port', 0) or 0)}</code></div>{endpoint_copy}</div>
      ...
      <section class="app-view" id="service-health" data-view="service-health" hidden>
        ...
        <section class="metric-grid service-health-summary" aria-label="Service health summary">
          <article class="metric"><div class="metric-icon">{_icon('health')}</div><div><small>Overall state</small><strong data-service-health-overall>{health_overall_label}</strong><span data-service-health-checked>{len(health_checks)} checks completed</span></div></article>
        ...
        <section class="panel service-health-panel"><div class="panel-heading"><div>{_icon('health')}<span><strong>Current checks</strong><small>Fresh RouterOS and dashboard readiness, without configuration changes</small></span></div></div><div class="service-health-list" data-service-health-list>{health_rows}</div></section>
      ...
      <section class="app-view" id="setup-planner" data-view="setup-planner" hidden>
        ...
        <section class="panel setup-planner-panel"><div class="panel-heading"><div>{_icon('shield')}<span><strong>Post-install verification</strong><small>One read-only go/no-go summary of the connected router. It never changes RouterOS or the container.</small></span></div><button type="button" class="primary" data-post-install-verify>{_icon('refresh')}<span>Run verification</span></button></div><div class="post-install-summary" data-post-install-summary hidden><i></i><span><strong></strong><small></small></span></div><ul class="setup-checks" data-post-install-checks><li><i></i><span>Run verification after installation or an immutable-image update to get a concise readiness summary.</span></li></ul><div class="post-install-next" data-post-install-next hidden><div><strong>Safe next steps</strong><ol data-post-install-actions></ol></div><div class="post-install-actions"><a class="quiet" href="https://github.com/aivanov2-godaddy/mikrotik-openvpn-gui/blob/main/docs/INSTALLATION_PLAYBOOK.md" target="_blank" rel="noreferrer">Open install guide</a><button type="button" class="quiet" data-copy-post-install>{_icon('copy')}<span>Copy next steps</span></button></div></div></section>
