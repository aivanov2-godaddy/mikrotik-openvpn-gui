...
def service_health_snapshot(
    *,
    ...
    if unavailable_reason:
        return {"overall": "unavailable", "checked_at": int(time.time()), "checks": [{"id": "routeros-rest", "name": "RouterOS REST connection", "status": "unavailable", "impact": "Live VPN health cannot be verified until the router responds.", "remediation": "Confirm the dashboard container can reach RouterOS REST over HTTPS, then refresh."}]}
    router, ovpn_server, certificate_settings, certificates = router or {}, ovpn_server or {}, certificate_settings or {}, certificates or []
    ...
    add("routeros-rest", "RouterOS REST connection", "healthy", "The dashboard can read the router and apply only reviewed changes.", "No action needed.")
    add("dashboard-storage", "Dashboard data store", "healthy" if database_ready else "unavailable", "Profiles, user controls, alerts, and change history are persistent." if database_ready else "Dashboard changes cannot be stored safely.", "No action needed." if database_ready else "Verify the container's persistent storage mount and available disk space, then refresh.")
    ...
    capacity = max(_used_percent(router.get("free-memory"), router.get("total-memory")), _used_percent(router.get("free-hdd-space"), router.get("total-hdd-space")), max(0, min(100, int(router.get("cpu-load", 0) or 0))))
    if capacity >= 95:
    ...
    return {"overall": "unavailable" if "unavailable" in statuses else ("warning" if "warning" in statuses else "healthy"), "checked_at": int(time.time()), "checks": checks}
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _users_with_metadata(
        ...
        for user in self.server.context.router.list_ovpn_users(credentials):
            ...
            values.append({**user, "email": emails.get(username, ""), "controls": controls,
                           "template": ({"id": template["id"], "name": template["name"], "group_name": template["group_name"], "overrides": assignment["overrides"]} if template and assignment else None)})
        return values
    ...
    def _service_health(self, credentials: RouterOSCredentials) -> dict[str, Any]:
        ...
        try:
            ...
            return service_health_snapshot(router=None, ovpn_server=None, certificate_settings=None, certificates=None, config=self.server.context.config, database_ready=False, unavailable_reason="router")
        try:
        ...
        try:
            ...
            certificates = []
        return service_health_snapshot(router=router, ovpn_server=ovpn_server, certificate_settings=certificate_settings, certificates=certificates, config=self.server.context.config, database_ready=database_ready)
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/policy-templates":
            ...
            if not session:
                return
            self._json({"templates": self.server.context.store.list_policy_templates(), "assignments": self.server.context.store.user_template_assignments()})
            return
        if path == "/api/users":
            session = self._require_session(api=True)
            if not session:
    ...
    def _dashboard(self) -> None:
        ...
        try:
            ...
            database_ready = False
        health = service_health_snapshot(router=router, ovpn_server=ovpn_server, certificate_settings=certificate_settings, certificates=certificates, config=self.server.context.config, database_ready=database_ready)
        self._html(
            dashboard_page(
                actor=session.username,
        ...
        try:
            ...
            if assignment:
                ...
                if template:
                    ...
                    overrides = [key for key in controls if controls.get(key) != template_controls.get(key)]
                    self.server.context.store.assign_policy_template(str(user["name"]), str(template["id"]), overrides=overrides)
            self.server.context.store.audit(
                actor=session.username,
                action="user.update",
    ...
    def _create_policy_template(self) -> None:
        ...
        try:
            ...
            template = self.server.context.store.save_policy_template(
                ...
                group_name=group_name, controls=self._parse_controls(data),
            )
            self.server.context.store.audit(actor=session.username, action="policy_template.create", target=template["id"], status="success", details={"name": name, "group": group_name})
    ...
    def _update_policy_template(self, template_id: str) -> None:
        ...
        try:
            ...
            name, description, group_name = self._template_identity(data)
            template = self.server.context.store.save_policy_template(template_id=template_id, name=name, description=description, group_name=group_name, controls=self._parse_controls(data, dict(existing.get("controls") or {})))
            self.server.context.store.audit(actor=session.username, action="policy_template.update", target=template_id, status="success", details={"name": name, "group": group_name})
    ...
    def _policy_template_action(self, template_id: str, action: str) -> None:
        ...
        try:
            ...
            if not template:
                raise ValueError("Policy template was not found")
            controls = self._parse_controls(dict(template.get("controls") or {}))
            credentials = self._credentials(session)
            users = {str(item.get("id", "")): item for item in self.server.context.router.list_ovpn_users(credentials)}
            if requested.difference(users):
                raise ValueError("One or more selected VPN users no longer exist; refresh and try again")
            keys = ("policy", "expires_at", "max_sessions", "rate_limit_kbps", "dns_mode", "notifications", "quota_mb", "schedule")
            preview = [{"id": user_id, "username": str(users[user_id]["name"]), "changes": [key for key in keys if self.server.context.store.user_controls(str(users[user_id]["name"])).get(key) != controls.get(key)]} for user_id in sorted(requested)]
            if action == "preview":
            ...
            for item in preview:
                if item["changes"]:
                    profile = self.server.context.router.ensure_rate_profile(credentials, username=item["username"], rate_limit_kbps=int(controls["rate_limit_kbps"]))
                    self.server.context.router.update_user(credentials, user_id=item["id"], profile=profile)
            ...
            self.server.context.store.audit(actor=session.username, action="policy_template.apply", target=template_id, status="success", details={"group": template["group_name"], "users": applied, "selected": len(preview)})
            self._json({"ok": True, "template": template, "applied": applied, "selected": len(preview)})
        except (ValueError, RouterOSError) as error:
            self.server.context.store.audit(actor=session.username, action=f"policy_template.{action}", target=template_id, status="failed", details={"reason": type(error).__name__})
            self._json({"error": str(error)}, status=HTTPStatus.BAD_REQUEST)
