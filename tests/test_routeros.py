...
class RouterOSClientTests(unittest.TestCase):
    ...
    def test_live_session_telemetry_and_termination(self) -> None:
        with MockRouterOS() as mock:
            ...
            self.assertEqual(active["rx_bytes"], 70001)
            self.assertEqual(active["tx_bytes"], 200001)
            self.assertEqual(active["rx_packets"], 401)
            self.assertEqual(active["tx_packets"], 901)
