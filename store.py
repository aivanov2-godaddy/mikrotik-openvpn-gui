...
class MetadataStore:
    ...
    def _built_in_templates() -> tuple[dict[str, Any], ...]:
        ...
        return (
            {"id": "standard", "name": "Standard", "group_name": "Employees",
             ...
             "controls": {"policy": "full-tunnel", "expires_at": None, "max_sessions": 5, "rate_limit_kbps": 0, "dns_mode": "router", "notifications": True, "quota_mb": 0, "schedule": "always"}},
            {"id": "contractor", "name": "Contractor", "group_name": "Contractors",
             ...
             "controls": {"policy": "lan-only", "expires_at": None, "max_sessions": 2, "rate_limit_kbps": 10240, "dns_mode": "router", "notifications": True, "quota_mb": 10240, "schedule": "weekdays"}},
    ...
    def list_policy_templates(self) -> list[dict[str, Any]]:
        with self._connection() as connection:
            rows = connection.execute("SELECT * FROM policy_templates ORDER BY protected DESC, name COLLATE NOCASE").fetchall()
    ...
    def policy_template(self, template_id: str) -> dict[str, Any] | None:
        with self._connection() as connection:
            row = connection.execute("SELECT * FROM policy_templates WHERE id=?", (str(template_id),)).fetchone()
        return self._template_row(row)

    def save_policy_template(self, *, template_id: str, name: str, description: str, group_name: str, controls: dict[str, Any]) -> dict[str, Any]:
        now = int(time.time())
        with self._lock, self._connection() as connection:
            existing = connection.execute("SELECT protected FROM policy_templates WHERE id=?", (str(template_id),)).fetchone()
            ...
            connection.execute(
                """INSERT INTO policy_templates(id, name, description, group_name, controls, protected, created_at, updated_at)
                   ...
                   ON CONFLICT(id) DO UPDATE SET name=excluded.name, description=excluded.description,
                       group_name=excluded.group_name, controls=excluded.controls, updated_at=excluded.updated_at""",
                (str(template_id), str(name), str(description), str(group_name), json.dumps(controls, separators=(",", ":"), sort_keys=True), now, now),
            )
        ...
        with self._lock, self._connection() as connection:
            connection.execute(
                """INSERT INTO user_policy_templates(vpn_user, template_id, overrides, assigned_at, updated_at)
                   VALUES (?, ?, ?, ?, ?)
                   ON CONFLICT(vpn_user) DO UPDATE SET template_id=excluded.template_id,
