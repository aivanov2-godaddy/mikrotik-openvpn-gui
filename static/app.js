...
function healthLabel(status) { return ({ healthy: 'Operational', warning: 'Attention needed', unavailable: 'Unavailable' }[status] || 'Unavailable'); }
function renderServiceHealth(payload) {
  const list = $('[data-service-health-list]'); if (!list) return;
  const checks = Array.isArray(payload.checks) ? payload.checks : [];
  list.replaceChildren(...checks.map((check) => { const row = node('article', `service-health-check ${check.status || 'unavailable'}`); row.append(node('span', 'health-check-led')); const copy = node('div'); copy.append(node('strong', '', check.name || 'Health check'), node('small', '', check.impact || 'No detail available.')); const next = node('p'); next.append(node('b', '', 'Safe next step: '), document.createTextNode(check.remediation || 'No action needed.')); copy.append(next); row.append(copy); return row; }));
  if (!checks.length) list.append(node('div', 'empty', 'No health data is available yet.'));
  const overall = $('[data-service-health-overall]'); if (overall) overall.textContent = healthLabel(payload.overall);
  const checked = $('[data-service-health-checked]'); if (checked) checked.textContent = `${checks.length} checks completed just now`;
  ['healthy', 'warning', 'unavailable'].forEach((status) => { const counter = $(`[data-service-health-count="${status}"]`); if (counter) counter.textContent = checks.filter((check) => check.status === status).length; });
}
async function refreshServiceHealth() { const button = $('[data-service-health-refresh]'); if (button) button.disabled = true; try { const response = await resultOrError(await api('/api/service-health')); renderServiceHealth(await response.json()); toast('Service health checks refreshed.'); } catch (error) { toast(error.message, 'error'); } finally { if (button) button.disabled = false; } }
...
$('#template-form')?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    toast(`${(await response.json()).template.name} is ready to preview.`);
    setTimeout(() => location.reload(), 450);
  } catch (error) { setStatus(form, error.message, true); setBusy(form, false); }
});

function selectedTemplateUsers(form) { return $$('input[name="user_ids"]:checked', form).map((input) => input.value); }
function showTemplatePreview(form, payload) {
  const preview = $('[data-template-preview]', form);
  preview.innerHTML = `<strong>${payload.template.name} preview</strong><small>${payload.changed_users} selected user(s) will change.</small><ul>${payload.users.map((user) => `<li><b>${user.username}</b><span>${user.changes.length ? user.changes.join(', ') : 'Already matches'}</span></li>`).join('')}</ul>`;
  preview.hidden = false;
  $('[data-template-apply-button]', form).disabled = false;
}
$('[data-template-preview-button]')?.addEventListener('click', async () => {
  const form = $('[data-template-apply]'); const templateId = form.template_id.value; const userIds = selectedTemplateUsers(form);
  if (!templateId || !userIds.length) { setStatus(form, 'Choose a template and at least one VPN user first.', true); return; }
  setBusy(form, true); setStatus(form, 'Comparing selected users with the template…');
  try { const response = await resultOrError(await api(`/api/policy-templates/${encodeURIComponent(templateId)}/preview`, { method: 'POST', body: { user_ids: userIds } })); showTemplatePreview(form, await response.json()); setStatus(form, 'Review the list, then apply if it is correct.'); }
  catch (error) { setStatus(form, error.message, true); } finally { setBusy(form, false); }
});
$('[data-template-apply]')?.addEventListener('submit', async (event) => {
  event.preventDefault(); const form = event.currentTarget; const templateId = form.template_id.value; const userIds = selectedTemplateUsers(form);
  if (!templateId || !userIds.length || $('[data-template-apply-button]', form).disabled) return;
  setBusy(form, true); setStatus(form, 'Creating a RouterOS checkpoint and applying the reviewed policy…');
  try { const response = await resultOrError(await api(`/api/policy-templates/${encodeURIComponent(templateId)}/apply`, { method: 'POST', body: { user_ids: userIds } })); const payload = await response.json(); toast(`Policy applied to ${payload.selected} selected user(s).`); setTimeout(() => location.reload(), 700); }
  catch (error) { setStatus(form, error.message, true); setBusy(form, false); }
});
